...
/**
 * Tells whether the contact form must wait for a Turnstile token before it can
 * submit. It reads the deployment-injected config through the shared Turnstile
 * module when the page loads it, and degrades to `false` (gate open) otherwise —
 * a site without a site key keeps a usable form.
 */
function resolveTurnstileRequired() {
    if (typeof globalThis !== 'undefined' && globalThis.__TURNSTILE_CONFIG__ &&
        typeof globalThis.__TURNSTILE_CONFIG__.siteKey === 'string' &&
        globalThis.__TURNSTILE_CONFIG__.siteKey.trim() !== '') {
        return true;
    }
    var resolver = (typeof window !== 'undefined' && window.TALARIA_TURNSTILE) || null;
    if (resolver && typeof resolver.isRequired === 'function') {
        return resolver.isRequired();
    }
    return false;
}

/**
 * Evaluates the Turnstile gate: when required, a non-blank token must be present.
 * `required` may be passed explicitly (tests); otherwise it is resolved.
 */
function turnstileSatisfied(options) {
    var opts = options || {};
    var required = Object.prototype.hasOwnProperty.call(opts, 'required')
        ? opts.required
        : resolveTurnstileRequired();
    if (!required) {
        return true;
    }
    return typeof opts.token === 'string' && opts.token.trim() !== '';
}
...
if (typeof document !== 'undefined' && document.addEventListener) {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (!turnstileSatisfied({ token: turnstileToken })) {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        ...
        submit: submit,
        channels: channels,
        resolveTurnstileRequired: resolveTurnstileRequired,
        turnstileSatisfied: turnstileSatisfied
    };
}
